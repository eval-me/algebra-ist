...
theorem ex_2_3_a {A : Type u} {G : group A} (x y z w : A) : 
  x * y * z⁻¹ * w = G.id → y = x⁻¹ * w⁻¹ * z := by
  intro h -- h turns into this x * y * z⁻¹ * w = G.id
  
  replace h := congrArg (λ a => a * w⁻¹) h -- apply inverse w
  ...
  replace h := congrArg (λ a => x⁻¹ * a) h -- apply inverse w
  dsimp at h -- remove the fun stuff!
  rw [<-G.mul_assoc, <-G.mul_assoc, G.inv_lmul, G.id_lmul] at h 

  replace h := congrArg (λ a => a * z) h -- apply inverse z
  dsimp at h -- remove the fun stuff!
  rw [G.mul_assoc, G.inv_lmul, G.id_rmul] at h
  
  trivial -- schiappa moment (ignore this teach)
